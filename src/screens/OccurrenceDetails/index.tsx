...
const OccurrenceDetails: React.FC = () => {
    ...
    const fetchData = () => {
        ...
        gunItems.forEach((gun) => {
            if (gun.value === occurrence.gun)
                setGun(gun.label);
        })
        victimItems.forEach((victim) => {
            if (victim.value === occurrence.victim)
                setVictim(victim.label);
        })
        ...
        <SafeAreaView style={{ flex: 1 }}>
            <Container>
                <HeaderTitle
                    ...
                    goBack
                />
                <KeyboardScrollView style={{width: '100%', alignSelf: 'center'}}>
                    <InputContainer style={{ width: "80%", marginTop: 0 }}>
                        <NormalLabel style={{ fontSize: scale(16)}} >Tipo de Ocorrência</NormalLabel>
